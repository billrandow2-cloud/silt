...
// Retorna a última semana disponível para um mês (quem "comanda")
function getLatestWeekForMonth(month) {
    const weeks = [...new Set(userPoolData.filter(p => p.month === month).map(p => p.week))];
    if (!weeks.length) return null;
    // Ordenar pela ordem das semanas e retornar a última
...
// Retorna dados apenas da última semana do mês
function getLatestWeekData(month) {
    const latestWeek = getLatestWeekForMonth(month);
    if (!latestWeek) return [];
    return userPoolData.filter(p => p.month === month && p.week === latestWeek);
}
...
function updateDashboardValues() {
    ...
    if (label) label.textContent = 'Referência: ' + lm + (latestWeek ? ' (' + latestWeek + ')' : '') + ' · ' + share.toFixed(0) + '% do lucro';
...
function renderWeeks(month) {
    ...
    // Resumo do mês - usa apenas a última semana como referência
    var latestWeekForResumo = sw[sw.length - 1]; // última semana disponível
    var latestWeekPools = weeks[latestWeekForResumo] || [];
